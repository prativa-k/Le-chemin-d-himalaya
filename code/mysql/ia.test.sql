...
INSERT INTO lechemin_test.orderable
...
INSERT INTO lechemin_test.menu (name, price) VALUES
...
INSERT INTO lechemin_test.orderable_menu (orderable_id, menu_id) VALUES
    (22,1),
    (44,1),
    (78,1);

INSERT INTO lechemin_test.orderable_menu (orderable_id, menu_id)
SELECT o.id, m.id
FROM lechemin_test.orderable o
JOIN lechemin_test.menu m ON m.name = 'Business Menu'
WHERE o.name IN (
  'Green Plate',
  'Samosa légume',
...
SELECT o.id, m.id
FROM lechemin_test.orderable o
JOIN lechemin_test.menu m ON m.name = 'Himalaya Menu'
WHERE o.name IN (
  'Soupe de lentille',
  'Poulet Tandoori Traditionnel',
  'Mix Pakora ',
...
SELECT o.id, m.id
FROM lechemin_test.orderable o
JOIN lechemin_test.menu m ON m.name = 'Menu Gastronomique'
WHERE o.name IN (
  'Achari murg tikka',
...
INSERT INTO lechemin_test.drink (name, price, drink_type_id)
VALUES
('Lassi Mangue', 4.50, 4),
('Thé Masala', 3.00, 3),
...
-- SELECT menu.*,GROUP_CONCAT(orderable.id) AS orderable_ids FROM lechemin_test.menu JOIN lechemin_test.orderable_menu ON orderable_menu.menu_id = menu.id JOIN lechemin_test.orderable ON orderable.id = orderable_menu.orderable_id GROUP BY menu.id;

-- SELECT category.*,GROUP_CONCAT(orderable.id) AS orderable_ids FROM lechemin_test.category JOIN lechemin_test.orderable ON category_id= category.id GROUP BY category.id;
