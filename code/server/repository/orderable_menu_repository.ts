import type { OrderableMenu } from "../../models/orderable_menu";
import MYSQLService from "../service/mysql_service";

class OrderableMenuRepository {
	//nom de la table SQL
	private table = "orderable_menu";

	// sélectionner tous les enregistrements
	public selectAll = async (): Promise<OrderableMenu[] | unknown> => {
	...
	public selectOne = async (
		data:Partial<OrderableMenu>,
	): Promise<OrderableMenu| unknown> => {
		// connexion au serveur MYSQL
		const connection = await new MYSQLService().connect();
		...
		try {
			...
			const result = (query as OrderableMenu[]).shift();
...
export default OrderableMenuRepository;
